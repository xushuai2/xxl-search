...
//			dodo();
	...
	private static void select() {
		List<Query> querys = new ArrayList<>();
		querys.add(new TermQuery(new Term("group", "xushuai")));
		Sort scoreSort = new Sort(new SortField("score", SortField.Type.INT, true));	// INT=按照int值排序
		SearchResult result = search(querys, scoreSort, 0, 20);
		for (Document item: result.getDocuments()) {
			System.out.println(item.get("hotscore"));
		}
	}
	...
	private static void dodo() {
		...
		doc.add(new IntField("id",1, Field.Store.YES));
		doc.add(new IntField("cityid", 1, Field.Store.YES));
		doc.add(new TextField("shopname", "文章内容", Field.Store.YES));
		doc.add(new StringField("group", "xushuai", Field.Store.YES));
		doc.add(new IntField("score", 5000, INT_FIELD_TYPE_STORED_SORTED));
		doc.add(new IntField("hotscore", 5000, INT_FIELD_TYPE_STORED_SORTED));
		addDocument(doc, true);
	}
